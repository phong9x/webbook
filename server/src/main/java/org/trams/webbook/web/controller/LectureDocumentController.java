 ...
 */
@Controller
@RequestMapping("/community/lecture&document")
public class LectureDocumentController extends AbstractController {
	...
	private static final String INDEX = "lecture_document/index";
	private static final String WRITE = "lecture_document/write";
	private static final String VIEW = "lecture_document/view";
	private static final String EDIT = "lecture_document/edit";
	private static String nav = "community";
	private static String menu = "lecture";
	// --- Main entity service
	@Resource
	 ...
	 * Default constructor
	 */
	public LectureDocumentController() {
		super(LectureDocumentController.class, MAIN_ENTITY_NAME);
		log("LectureDocumentController created.");
	}

	// --------------------------------------------------------------------------------------
	// Spring MVC model management
	// --------------------------------------------------------------------------------------
	//Decription:lecture&document
	//Url: 		http://kwebsosul.com/community/lecture&document
	@RequestMapping("")
	public String index(
		...
		try {
			list = postService.findByBulletin(ApplicationDefine.BULLETIN_LECTURE_DOCUMENT, page, PAGE_SIZE, "createDate");
	...
	public String report(
		...
		try {
			...
			if(user != null){
				...
				Report report = reportService.findByUserParrentType(user.getId(), post.getId(), ApplicationDefine.REPORT_LECTURE_DOCUMENT);
				if(report == null){
					report = new Report();
					report.setId(0);
					...
					report.setTypeKey(ApplicationDefine.REPORT_LECTURE_DOCUMENT);
				...
				return "redirect:/community/lecture&document/view/"+post.getId();
		...
		return "redirect:/community/lecture&document";
	}
	
	//Decription: view detail lecture&document page
	//Url: 		 http://kwebsosul.com/community/lecture&document/view/{id}
	...
	//Decription: edit lecture&document
	//Url: 		http://kwebsosul.com/community//edit/{id}
	...
	public String editIndex(
		...
		try {
			...
			if(post == null || Utils.parseInt(editor.getId())  != Utils.parseInt( post.getUser().getId())){
				return "redirect:/community/lecture&document";
			}
			...
			if(user != null){
				if(title != null && content != null){
					...
					try {
						...
						if(editPost != null){
							return "redirect:/community/lecture&document/view/"+editPost.getId();
						}else{
					...
					return "redirect:/community/lecture&document/view/"+id;
				}
			}else{
				return "redirect:/login";
			}
	...
	//Decription: create lecture&document
	//Url: 		http://kwebsosul.com/community/write
		...
		try {
			...
			if(title != null && content != null){
				...
				if(user != null){
					...
					post.setIsDelete(0);
					post.setBulletinId(ApplicationDefine.BULLETIN_LECTURE_DOCUMENT);
					post.setBulletinName(ApplicationDefine.BULLETIN_LECTURE_DOCUMENT_NAME);
					post.setUpdateDate(new Date());
				...
				try {
					...
					if(createNew != null){
						return "redirect:/community/lecture&document/view/"+createNew.getId();
					}
				}
			}else{
	...
	//Decription: report comment in lecture&document page
	...
	public String report(
		...
		try {
			...
			if(user != null){
				...
				return "redirect:/community/lecture&document/view/"+episode_id;
